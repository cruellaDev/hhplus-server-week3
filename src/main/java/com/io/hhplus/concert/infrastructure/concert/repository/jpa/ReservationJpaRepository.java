...
public interface ReservationJpaRepository extends JpaRepository<ReservationEntity, Long> {
    @Query("SELECT R FROM ReservationEntity R WHERE R.customerId =:customerId AND R.deletedAt IS NULL AND (SELECT COUNT(T) FROM TicketEntity T WHERE T.reservationId = R.id AND T.concertId =:concertId AND T.concertScheduleId =:concertScheduleId AND T.seatNumber IN :seatNumbers AND T.cancelApprovedAt IS NULL AND T.deletedAt IS NULL) > 0")
